...
Bigger work gets a task file in `work/`. Bigger means it spans more than one session, or {{OWNER}} asked for it as a feature. A quick fix skips the task file and only updates the page it touched.

1. Copy the [task template](task-template.md) to `work/short-name.md`.
2. Copy what {{OWNER}} asked for, in the exact words used. Do not tidy them.
...
- It has five headings, always in this order: In flight, Waiting on {{OWNER}}, Next up, Parked, Watch out.
...
- An old line is a question, not a fact. If something has been in flight for more than a week, check whether the branch still exists and whether the work landed. Then fix the line, or ask {{OWNER}}.
...
- Pull before you write. Push right after you commit, if this repository has a remote and {{OWNER}} allows it.
...
- Pushing the code repository is a separate matter. Follow {{OWNER}}'s rule for it.
...
- A fact about working with {{OWNER}} goes here too, in `rules/`. Every harness needs it, not only the one that heard it.
